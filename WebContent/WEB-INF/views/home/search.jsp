		...
		<div class="row">
			...
			<div class="col-md-9">
				<c:if test="${songs.size() > 0 }">
					<!-- Search Song Result -->
					<h3 class="title-list-index mb-2">SONG</h3>
						<c:forEach var="song" items="${songs }">
							...
							<div class="row">
								<div class="col-md-9">
									<a href="${playSong }" class="linksong">
										${song.name }
									</a>
									-
									<c:forEach items="${song.songDetails }" var="songDetail" varStatus="i">
				...
				<c:if test="${videos.size() > 0 }" >
					<!-- Search Video Result -->
					<h3 class="title-list-index mt-3 mb-2">VIDEO</h3>
									...
									<c:forEach items="${videos }" var="video">
										<c:if test="${temp == 1 }">
											<div class="col-md-6">
												<div class="row">
													<div class="col-md-4">
														<a href=""> <img src="${pageContext.request.contextPath }/assets/images/${video.videoPhoto }"
															class="img-thumbnail">
														</a>
													</div>
													<div class="col-md-8">
														...
														<a href="${playVideo }" class="song-name2"> 
															<b>${video.name }</b>
														</a> 
														<br> 
														<span>
															<c:forEach items="${video.songDetails }" var="songDetail" varStatus="i">
																<c:url var="singerLink" value="/artist/singer/info.html">
																	<c:param name="id" value="${songDetail.singer.id }" />
																</c:url>
																<span>
																	...
																	<c:if test="${i.index < video.songDetails.size() - 1 }">
																		,&nbsp;
																	</c:if>
																</span>
															</c:forEach>
				...
				<c:if test="${singers.size() > 0 }" >
					...
					<h3 class="title-list-index mt-3 mb-2">SINGER</h3>
					<c:forEach begin="0" end="1" var="i">
							<c:set var="temp" value="${i % 2 }" />
							<c:if test="${temp == 1 }">
								<div class="row mb-3">
									<c:forEach items="${singers }" var="singer">
										<div class="col-md-6">
											<div class="row">
												...
												<div class="col-md-4">
													<a href="${singerLink }"> 
														<img src="${pageContext.request.contextPath }/assets/images/${singer.photo }"
														class="img-thumbnail">
													</a>
												</div>
												<div class="col-md-8">
													<a href="${singerLink }" class="song-name2"> 
														<b>${singer.name }</b>
													</a>
													<p>
														${fn:substring(singer.description, 0, 135) }...
													</p>
												</div>
											</div>
										</div>
